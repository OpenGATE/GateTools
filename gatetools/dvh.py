...
def createDVH(dose=None, roi=None):
    ...
    statsRoiFilter.SetHistogramParameters(100, statsRoiFilter.GetMinimum(1), statsRoiFilter.GetMaximum(1))
    statsRoiFilter.Update()
    histogramRoi = statsRoiFilter.GetHistogram(1)

    i = 0
    j = 1
    doseValues = []
    volumePercentage = np.array(list(range(0, 101)))/100
    while i < len(volumePercentage):
        doseValues.append(histogramRoi.Quantile(0, j))
        i = i + 1
        j = j - 0.01
...
class Test_DVH(LoggedTestCase):
    def test_dvh(self):
        ...
        self.assertTrue(np.allclose(volumePercentage, np.array(list(range(0, 101)))/100))
